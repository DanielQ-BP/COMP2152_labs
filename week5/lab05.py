...
if not input_invalid:
  ...
  @        ..@#% @     @&*#@(         % 
   &   (  @    (   / /   *    @  .   /  
    ...
    # Validation and Input of Hero name
    while True:
        ...
        if len(name_parts) == 2 and all(part.isalpha() for part in name_parts):
            ...
            print("Invalid input! Please enter exactly two words containing only letters.")

    # Generates the short name
    short_name = name_parts[0][:2] + name_parts[1][:1]

    # Prints the final stars with the short name
    stars_display = "*" * num_stars
    print(f"Hero {short_name} gets <{stars_display}> stars")

    def collect_loot(loot_options, belt):
        """Rolls for loot and adds items to the player's belt."""
        print("!!You find a loot bag!! You loook inside to find 2 items:")
        for _ in range(2):
            ...
            # Organizes the belt alphabetically
        print("You're super neat, so you organize your belt alphabetically:")
        belt.sort()
        print("Your belt:", belt)
    ...
    def use_loot(belt, health_points, good_loot_options, bad_loot_options):
        """Uses the first item in the belt and adjusts health points accordingly."""
        print("!!You see a monster in the distance! So you quickly use your first item:")
        first_item = belt.pop(0)
    ...
    def hero_attacks(combat_strength, m_health_points):
        """Hero attacks the monster and reduces its health points."""
        attack_damage = random.choice(range(1, combat_strength + 1))
        m_health_points = max(0, m_health_points - attack_damage)
        print(f"Hero attacks for {attack_damage} damage! Monster health is now {m_health_points}")
        return m_health_points

    def monster_attacks(m_combat_strength, health_points):
        """Monster attacks the hero and reduces their health points."""
        attack_damage = random.choice(range(1, m_combat_strength + 1))
        health_points = max(0, health_points - attack_damage)
        print(f"Monster attacks for {attack_damage} damage! Hero health is now {health_points}")
        return health_points

    def inception_dream(crazy_level=0):
        """Recursive function that deepens the dream layers and increases crazy_level."""
